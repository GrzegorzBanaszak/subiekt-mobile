...
import { createOrder, getAvailableOrderAssignees, publishOrder, searchProducts, type ProductListItem } from '../api/ordersApi'
import { formatOrderNumber } from '../orderFormat'
...
export function NewOrderPage() {
  ...
  const formatWeight = (value: number) => `${formatOrderNumber(value, language)} kg`

  return <section className="mx-auto max-w-[1200px]">
    <div className="mb-6 flex items-center gap-3"><Link aria-label={t('orders.details.back')} to="/orders" className="flex size-11 items-center justify-center rounded-full hover:bg-slate-200"><AppIcon name="arrowBack" /></Link><div><h2 className="text-2xl font-bold">{t('orders.new.title')}</h2><p className="text-sm text-slate-600">{t('orders.status.draft')}</p></div></div>
