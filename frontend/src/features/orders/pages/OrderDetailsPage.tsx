...
import { formatOrderDate, formatOrderDateTime, formatOrderNumber, isPublishedOrder } from '../orderFormat'
...
export function OrderDetailsPage() {
  ...
  const confirmDelete = () => window.confirm(t('orders.details.confirmDelete').replace('{number}', order.number))

  return <section className="mx-auto max-w-[1200px]">
    <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div className="flex items-start gap-3"><Link aria-label={t('orders.details.back')} to="/orders" className="flex size-11 items-center justify-center rounded-full hover:bg-slate-200"><AppIcon name="arrowBack" /></Link><div><p className="text-sm font-semibold text-slate-500">{t('orders.details.eyebrow')}</p><h2 className="text-2xl font-bold lg:text-3xl">{order.number}</h2><p className="mt-1 text-sm text-slate-600">{t('orders.details.lastChange').replace('{date}', formatOrderDateTime(order.updatedAtUtc, language))}</p></div></div><div className="flex flex-wrap items-center gap-2"><span className={`w-fit rounded-full px-3 py-1.5 text-sm font-semibold ${published ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-200 text-slate-700'}`}>{t(published ? 'orders.status.ready' : 'orders.status.draft')}</span>{!published && <><button className="min-h-11 rounded-lg border border-red-300 bg-white px-4 font-semibold text-red-700 hover:bg-red-50 disabled:opacity-50" disabled={remove.isPending || publish.isPending} onClick={() => { if (confirmDelete()) remove.mutate(Number(order.version)) }}>{t('orders.details.delete')}</button><button className="min-h-11 rounded-lg bg-blue-950 px-4 font-semibold text-white hover:bg-blue-900 disabled:opacity-50" disabled={remove.isPending || publish.isPending} onClick={() => publish.mutate(Number(order.version))}>{publish.isPending ? t('orders.details.publishing') : t('orders.details.publish')}</button></>}</div></div>
